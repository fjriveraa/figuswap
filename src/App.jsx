...
const MOCK_CHATS = {
  ...
  FWC:[{user:"Kenji T.",emoji:"🇯🇵",msg:"FWC 4 es muy difícil!",time:"08:45"},{user:"James K.",emoji:"🇬🇧",msg:"Yo la tengo repetida, $3 USD",time:"08:50"}],
};
...
const sbAuth = {
  ...
  getSessionFromHash() {
    ...
    if(token) { window.location.hash=""; return {token,email}; }
    return null;
  },
  getStoredSession() {
    try {
      const s = localStorage.getItem("figuswap_session");
      return s ? JSON.parse(s) : null;
    } catch { return null; }
  },
  storeSession(session) {
    try { localStorage.setItem("figuswap_session", JSON.stringify(session)); } catch {}
  },
  clearSession() {
    try { localStorage.removeItem("figuswap_session"); } catch {}
  }
};
  ...
  const handleEmail = async () => {
    ...
    try {
      if(mode==="login"){
        ...
        if(r.access_token){
          const session={token:r.access_token,email:r.user?.email||email};
          sbAuth.storeSession(session);
          onAuth(session);
    ...
    <div style={{minHeight:"100vh",background:"#0a0f1e",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20}}>
      ...
      <div style={{background:"#111827",border:"1px solid #1e2a3a",borderRadius:16,padding:24,width:"100%",maxWidth:380}}>
        ...
        <button
          ...
          style={{width:"100%",padding:"14px",background:"#fff",border:"1px solid #e5e7eb",borderRadius:10,color:"#1f2937",fontWeight:700,fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10,marginBottom:16}}
        >
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
          ...
          <span style={{fontSize:12,color:"#4a5568"}}>o con email</span>
          <div style={{flex:1,height:1,background:"#1e2a3a"}}/>
        </div>
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#000c",zIndex:300,display:"flex",flexDirection:"column"}}>
      <div style={{background:"#111827",borderBottom:"1px solid #1e2a3a",padding:"14px 16px",display:"flex",alignItems:"center",gap:10}}>
        ...
        <div><div style={{fontWeight:800,color:"#e8eaf6"}}>Chat {team?.name}</div><div style={{fontSize:11,color:"#6b7280"}}>{msgs.length} mensajes</div></div>
      </div>
...
export default function FiguSwap() {
  ...
  // Load stickers from localStorage per user
  useEffect(()=>{
    if(!session)return;
    try{
  ...
  const NAV=[["album","📋","Álbum"],["marketplace","🏪","Market"],["profile","👤","Perfil"]];
      ...
      <div style={{maxWidth:720,margin:"0 auto",padding:16}}>
        ...
        {page==="marketplace"&&(
          <div>
            ...
            <p style={{color:"#6b7280",fontSize:13,marginBottom:16}}>Figuritas disponibles de coleccionistas del mundo</p>
            <div style={{background:"#111827",border:"1px solid #1e2a3a",borderRadius:14,padding:16,marginBottom:16}}>
              <div style={{fontWeight:700,color:"#ffd700",marginBottom:10}}>📤 Mis publicaciones activas</div>
              {(()=>{
                const my=[];
                Object.entries(stickers).forEach(([code,ts])=>{Object.entries(ts).forEach(([num,s])=>{if(["sell","trade","auction","repeated"].includes(s.state))my.push({code,num:parseInt(num),...s});});});
                if(my.length===0)return <div style={{color:"#4a5568",fontSize:13}}>Ninguna. Marca figuritas en tu álbum como 💰 Venta, 🔄 Cambio o 🔨 Subasta.</div>;
                return <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{my.slice(0,20).map((l,i)=>(<span key={i} style={{padding:"4px 10px",borderRadius:20,background:STATE[l.state].bg,border:`1px solid ${STATE[l.state].color}`,color:STATE[l.state].color,fontSize:12,fontWeight:700}}>{STATE[l.state].emoji} {l.code} #{l.num}{l.price>0?` $${l.price}`:""}</span>))}</div>;
              })()}
            </div>
            <div style={{background:"#111827",border:"1px dashed #1e2a3a",borderRadius:14,padding:24,textAlign:"center"}}>
              <div style={{fontSize:32,marginBottom:8}}>🌍</div>
              <div style={{fontWeight:700,color:"#e8eaf6",marginBottom:4}}>Marketplace en construcción</div>
              <div style={{fontSize:13,color:"#6b7280"}}>Pronto verás figuritas de otros coleccionistas aquí.</div>
            </div>
          </div>
        )}
