  ...
  if(step==="upload") return (
    <div style={{padding:16,maxWidth:480,margin:"0 auto"}}>
      ...
      <p style={{color:"#6b7280",fontSize:13,marginBottom:20}}>
        Sube una foto y la IA detecta automáticamente qué figuritas hay.
      </p>
      ...
      <div style={{background:"#111827",border:"1px solid #1e2a3a",borderRadius:14,padding:16,marginBottom:20}}>
        {[
          ...
          ["3️⃣","Decide si es Entrada (lo que tienes) o Salida (lo que entregaste)"],
        ].map(([ic,txt],i)=>(
  ...
  return (
    <div style={{padding:16,maxWidth:480,margin:"0 auto"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
        ...
        <h2 style={{fontWeight:900,fontSize:18,color:"#ffd700",margin:0}}>{mode==="salida"?"📤 Dar de baja":"Resultado del escaneo"}</h2>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
        <button onClick={()=>setMode("entrada")} style={{padding:"12px 8px",borderRadius:12,border:mode==="entrada"?"2px solid #22c55e":"1px solid #1e2a3a",background:mode==="entrada"?"#052e16":"#111827",color:mode==="entrada"?"#86efac":"#6b7280",fontWeight:800,fontSize:13,cursor:"pointer"}}>
          📥 Entrada
          <div style={{fontSize:10,fontWeight:500,marginTop:2,opacity:0.8}}>Esto es lo que tengo</div>
        </button>
        <button onClick={()=>setMode("salida")} style={{padding:"12px 8px",borderRadius:12,border:mode==="salida"?"2px solid #ef4444":"1px solid #1e2a3a",background:mode==="salida"?"#1e0a0a":"#111827",color:mode==="salida"?"#fca5a5":"#6b7280",fontWeight:800,fontSize:13,cursor:"pointer"}}>
          📤 Salida
          <div style={{fontSize:10,fontWeight:500,marginTop:2,opacity:0.8}}>Esto entregué</div>
        </button>
      </div>

      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:16}}>
        {(mode==="salida"?[
          ["📤",detected.filter(s=>s.availableQty>0).reduce((sum,s)=>sum+Math.min(s.quantity,s.availableQty),0),"#ef4444","Por dar salida"],
          ["⚠️",detected.filter(s=>s.availableQty===0).length,"#f97316","Sin stock"],
      ...
      {mode==="salida"&&detected.length>0?(
        <div style={{marginBottom:20}}>
          <div style={{fontWeight:800,fontSize:15,color:"#ef4444",marginBottom:4}}>
            📤 Dar de baja ({detected.length})
          </div>
          <div style={{fontSize:12,color:"#6b7280",marginBottom:10}}>
            Esto resta del stock que tenías registrado como disponible (repetida/venta/cambio/subasta).
          </div>
          {detected.map((s,i)=>{
              ...
              <div key={i} style={{background:overScanned&&!done?"#1e1500":"#111827",border:overScanned&&!done?"1px solid #f97316":"1px solid #1e2a3a",borderRadius:12,padding:14,marginBottom:8,display:"flex",alignItems:"center",gap:10}}>
                ...
                <div style={{flex:1}}>
                  <div style={{fontWeight:800,color:"#e8eaf6"}}>
                    ...
                    {s.quantity>1&&<span style={{marginLeft:8,fontSize:11,background:"#ef4444",color:"#fff",padding:"2px 8px",borderRadius:10,fontWeight:700}}>x{s.quantity} escaneadas</span>}
                  </div>
                  {s.availableQty>0
                    ? <div style={{fontSize:12,color:"#9ca3af"}}>Tenías {s.availableQty} disponible{s.availableQty>1?"s":""} registrada{s.availableQty>1?"s":""}</div>
                    : <div style={{fontSize:12,color:"#fb923c",fontWeight:700}}>⚠️ No tenías esta marcada como disponible — revisa antes de confirmar</div>
                  }
                </div>
                {done
                  ? <span style={{fontSize:22}}>✅</span>
                  : s.availableQty>0
                    ? <button onClick={()=>giveOut(s)} style={{padding:"8px 14px",background:"#ef4444",border:"none",borderRadius:8,color:"#fff",fontWeight:700,fontSize:12,cursor:"pointer",whiteSpace:"nowrap"}}>📤 Dar salida</button>
